                ...
                int use_name, int use_weight)
{
  ...
  /* partitions around the pivot value and swaps accordingly compares weights*/
  if (use_weight == 1)
  {
...
void insertion_sort_internal(monster *list, int n, int *comparisons, int *copies, int *block_copies, int use_name, int use_weight)
{
  ...
  for (i = 1; i < n; i++)
  {
    ...
    // output_monster_list(list, n, "unordered list");
                                    ...
                                    int use_name, int use_weight, FILE *ofp)
{
  ...
  // printf("high_index: %d, low_index: %d, n: %d\n", high_index, low_index, n);
  // printf("n : %d\n", n);
  int half = (low_index + high_index) / 2;
  // printf("n : %d\n", n);

  // int auxArraySize = high_index - low_index + 1;

  // monster listAux[high_index - low_index + 1];

  // memcpy(listAux, list + low_index, auxArraySize * sizeof(monster));

  if (n <= 25)
  {

    // output_monster_list(list, n, "insertion unsorted list:");
    int i;
    monster key;
    int position;
    ...
    // output_monster_list(list, n, "insertion sorted list:");
  ...
  else
  {
    ...
    // malloc arrays to pass to the arguments, that depend on the low index and high index, memmove or memcpy to that empty array
    // pass the list to the argument and sort, merge sort might break because of this.
...
// if condition in the insertion sort to check if there is any change if there isn't you don't block copy anything.